    ...
    <div id="container">
        ...
        <?php include '../../include/sildeber-left.php';?>
            <div style="height:100vh" class="sidebar-right">
                <h1>Danh Sách Tài Khoản Cần Duyệt</h1>
            <?php
                $Permission=0;
                $stmt = $conn->prepare("SELECT * FROM taikhoan where ID!=? and Permission=? order by DaTe DESC");
                $stmt->bind_param("ii",$_SESSION["ID"],$Permission);
                $stmt->execute();
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="delete_acount.php?ID=<?php echo $row["ID"]?>&taikhoancanduyet=1"><i class="fa-solid fa-trash"></i></a>
                        </td>
                    </tr>
                    <?php
            }
